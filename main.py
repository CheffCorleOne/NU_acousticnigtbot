...
(MAIN_MENU, EDIT_PROFILE, SELECT_INSTRUMENTS, SELECT_SEEKING, WRITE_BIO,
 BROWSE_MODE, BROWSE_PROFILES) = range(7)

INSTRUMENTS = [
    "vocalist", "guitarist", "drummer", "cajon player", "sound engineer",
    "pianist", "bassist", "violinist", "other instrument",
    "other (non-instrumental)"
]
...
class Database:
    ...
    def get_user(self, user_id):
        try:
            with self.conn.cursor() as cur:
                cur.execute("SELECT data FROM users WHERE user_id = %s",
                            (user_id, ))
                result = cur.fetchone()
                if result:
                    return json.loads(result[0]) if isinstance(
...
class AcousticMatchBot:
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        user_data.update({"name": user.full_name, "username": user.username})
    ...
    async def main_menu(self, update: Update,
        ...
        keyboard = [
            [
                InlineKeyboardButton("✏️ Edit Profile",
                                     callback_data="edit_profile")
            ],
            [
                InlineKeyboardButton("🔍 Find Collaborators",
                                     callback_data="browse_mode")
            ],
        ...
        if update.message:
            await update.message.reply_text(
                "🎸🎶 Hey there! This is Acoustic Night bot for collaborations! Click Help to find out more! 🌟: ",
    ...
    async def edit_profile(self, update: Update,
                           context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        await query.answer()

        keyboard = [[
            InlineKeyboardButton("🎻 My Instruments",
                                 callback_data="edit_instruments")
        ], [InlineKeyboardButton("🔍 Seeking", callback_data="edit_seeking")],
                    [InlineKeyboardButton("📝 Bio", callback_data="edit_bio")],
                    [InlineKeyboardButton("🔙 Back", callback_data="back")]]

        await query.edit_message_text(
            "✏️ Edit Profile:", reply_markup=InlineKeyboardMarkup(keyboard))
        return EDIT_PROFILE

    async def select_category(self, update: Update,
                              context: ContextTypes.DEFAULT_TYPE,
                              category: str):
        query = update.callback_query
        await query.answer()
    ...
    async def handle_toggle(self, update: Update,
                            context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        user_id = str(query.from_user.id)
    ...
    async def request_bio(self, update: Update,
                          context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        await query.answer()
        await query.edit_message_text(
    ...
    async def save_bio(self, update: Update,
                       context: ContextTypes.DEFAULT_TYPE):
        ...
        if len(bio) > 120:
            await update.message.reply_text(
                "❌ Bio is too long! Maximum 120 characters.")
            return WRITE_BIO
    ...
    async def browse_mode(self, update: Update,
                          context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        await query.answer()
    ...
    async def prepare_browsing(self, update: Update,
                               context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        await query.answer()
        user_id = str(query.from_user.id)
        ...
        for user in all_users:
            ...
            if mode == "smart":
                instrument_match = any(instr in user["instruments"]
                                       for instr in current_user["seeking"])
                seeking_match = any(instr in current_user["instruments"]
                                    for instr in user["seeking"])
                if instrument_match and seeking_match:
                    candidates.append(user)
            else:
                # Include all users regardless of previous interactions
                candidates.append(user)
    ...
    async def show_profile(self, update: Update,
        ...
        contact = f"@{profile['username']}" if (
            is_match and
            profile.get("username")) else "🔒 Contact hidden until mutual match"
        ...
        # Always show "Let's Collab" button regardless of previous interactions
        keyboard.append([
        ...
        text = (f"🎸 Profile {index+1}/{len(candidates)}\n\n"
                f"👤 Name: {profile['name']}\n"
                f"📞 Contact: {contact}\n"
    ...
    async def handle_navigation(self, update: Update,
                                context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        action = query.data
    ...
    async def handle_like(self, update: Update,
                          context: ContextTypes.DEFAULT_TYPE):
        query = update.callback_query
        user_id = str(query.from_user.id)
        current_user = self.db.get_user(user_id)
        profile = context.user_data["candidates"][
            context.user_data["current_index"]]
        target_id = profile["user_id"]

        # Remove check for existing pending request
    ...
    async def handle_response(self, update: Update,
        ...
        if action == "accept":
            ...
            contact = f"@{sender_data['username']}" if sender_data.get(
                "username") else "⚠️ No username set"
            text = (f"🎉 New Collaboration Partner!\n\n"
    ...
    async def show_matches(self, update: Update,
        ...
        smart_matches = [
            u["user_id"] for u in all_users if u["user_id"] != user_id and any(
                instr in u["instruments"]
                for instr in current_user["seeking"]) and any(
                    instr in current_user["instruments"]
                    for instr in u["seeking"])
        ]
        ...
        for match_id in all_matches:
            ...
            contact = f"@{match['username']}" if match.get(
                "username") else "⚠️ No username set"
            matches_text += (
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            BROWSE_PROFILES: [
                CallbackQueryHandler(bot.handle_navigation,
    ...
    # Обработчик ответов на запросы о коллаборации
    application.add_handler(
        CallbackQueryHandler(bot.handle_response,
